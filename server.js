...
/* ============================================================================
   ...
   Node.js + Express backend. Run: node server.js
   ============================================================================ */
...
/* ---------- DATA ---------- */
let db = { users: [], adminSessions: {}, events: [], meta: { created: new Date().toISOString(), version: 1 } };
...
function loadDB() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      console.log('[XYVEN] No datastore found. Creating fresh at ' + DATA_FILE);
    }
  } catch (err) {
...
function pruneData() {
  ...
  Object.keys(db.adminSessions).forEach((token) => {
    if (!db.adminSessions[token] || db.adminSessions[token].expires < now) {
      delete db.adminSessions[token]; changed = true;
    }
  });
  db.users.forEach((u) => { if (u.otp && u.otp.expires < now) { u.otp = null; changed = true; } });
  if (db.events.length > 5000) { db.events = db.events.slice(-3000); changed = true; }
  if (changed) saveDB();
}
...
function hmac(value) { return crypto.createHmac('sha256', SESSION_SECRET).update(String(value)).digest('hex'); }
...
function verifyPassword(password, stored) {
  ...
  try { return safeEqual(crypto.scryptSync(password, parts[0], 64).toString('hex'), parts[1]); }
  catch (e) { return false; }
}
function signValue(value) { return value + '.' + hmac(value).slice(0, 40); }
...
function clearCookie(res, name) { setCookie(res, name, '', { maxAge: 0, httpOnly: true }); }
...
function recordEvent(label, kind, userId, req) {
  ...
  const evt = { id: randomId(8), label: label, kind: kind || 'ok', at: new Date().toISOString(), userId: userId || null, actor: actor };
  db.events.push(evt);
...
const apiLimiter = rateLimit({ windowMs: 60 * 1000, limit: 180, standardHeaders: 'draft-7', legacyHeaders: false, message: { error: 'rate_limited', message: 'Too many requests.' } });
const otpLimiter = rateLimit({ windowMs: 10 * 60 * 1000, limit: 12, standardHeaders: 'draft-7', legacyHeaders: false, message: { error: 'rate_limited', message: 'Too many verification attempts.' } });
const loginLimiter = rateLimit({ windowMs: 15 * 60 * 1000, limit: 8, standardHeaders: 'draft-7', legacyHeaders: false, skipSuccessfulRequests: true, message: { error: 'rate_limited', message: 'Too many login attempts.' } });
...
if (!ADMIN_PASSWORD_HASH) {
  ...
  if (!process.env.ADMIN_PASSWORD) {
    console.warn('[XYVEN] WARNING: Using default admin password.');
  }
}
...
function createUserSession() {
  ...
  const user = {
    id: randomId(8), sessionId: sessionId,
    consent: null, phone: null, phoneVerified: false,
    otp: null, otpRequests: [], device: null,
...
function ensureUserSession(req, res) {
  ...
  if (!user) {
    ...
    setCookie(res, COOKIE_USER, signValue(user.sessionId), { maxAge: SESSION_TTL_MS, httpOnly: true, sameSite: 'Lax' });
...
function isAdmin(req) {
  ...
  if (record.expires < Date.now()) { delete db.adminSessions[token]; saveDB(); return null; }
  return record;
}
function requireAdmin(req, res, next) {
  const admin = isAdmin(req);
  if (!admin) return res.status(401).json({ error: 'unauthorized', message: 'Administrator authentication required.' });
  req.admin = admin;
  next();
}
...
/* ============================================================================
   ADMIN HTML — dynamically generated
   ============================================================================ */
...
'.alarm-overlay{position:fixed;inset:0;z-index:58;pointer-events:none;background:radial-gradient(circle at center,rgba(255,0,20,.15) 0%,rgba(255,0,20,.6) 100%);opacity:0;transition:opacity .15s}',
'.alarm-overlay.on{animation:af .28s steps(2) 5}',
'@keyframes af{0%,100%{opacity:.8}50%{opacity:.15}}',
'.alarm-badge{position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:59;padding:14px 30px;border:2px solid #ff2b3d;background:rgba(20,0,4,.94);color:#fff;font-size:15px;letter-spacing:.3em;font-weight:800;box-shadow:0 0 50px rgba(255,43,61,.85),0 0 100px rgba(255,43,61,.5);text-shadow:0 0 14px #ff2b3d;pointer-events:none;display:none;white-space:nowrap}',
'.alarm-badge.on{display:block;animation:abi .25s ease}',
'@keyframes abi{from{opacity:0;transform:translateX(-50%) scale(.9)}to{opacity:1;transform:translateX(-50%) scale(1)}}',
...
'<div class="alarm-badge" id="alarmBadge">⚠ SECURITY ALERT ⚠</div>',
'',
...
'      <p class="small muted" style="margin-top:18px">Credentials are verified server-side with scrypt hashing.</p>',
'    </div>',
'  </div>',
'</section>',
'',
...
'      XYVEN COMMAND CENTER — authorised administrators only. All actions are audit-logged.',
'    </p>',
'  </div>',
'</section>',
'',
...
'function playAlarm(){try{var AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;var ctx=new AC();var now=ctx.currentTime;for(var i=0;i<5;i++){var t0=now+i*0.32;var osc=ctx.createOscillator();var gain=ctx.createGain();osc.type="square";osc.frequency.setValueAtTime(i%2===0?920:640,t0);gain.gain.setValueAtTime(0.001,t0);gain.gain.exponentialRampToValueAtTime(0.14,t0+0.02);gain.gain.exponentialRampToValueAtTime(0.001,t0+0.28);osc.connect(gain);gain.connect(ctx.destination);osc.start(t0);osc.stop(t0+0.30);}setTimeout(function(){try{ctx.close();}catch(e){}},2200);}catch(e){}}',
'function triggerAlarm(msg){playAlarm();if(RM)return;var ov=$("#alarmOverlay"),bd=$("#alarmBadge");if(ov){ov.classList.remove("on");void ov.offsetWidth;ov.classList.add("on");setTimeout(function(){ov.classList.remove("on");},1600);}if(bd){if(msg)bd.textContent=msg;bd.classList.add("on");setTimeout(function(){bd.classList.remove("on");},2400);}shake();}',
'function screen(id){',
...
'var INTRO_C=["SYSTEM IS READY.","COMMAND CENTER IS ONLINE.","LET\x27S BREAK THE LIMITS."];',
...
'  err.textContent="";',
'  if(!u||!p){err.textContent="Username and password are required.";triggerAlarm("⚠ CREDENTIALS REQUIRED");return;}',
...
'    triggerAlarm("⚠ ACCESS DENIED — INVALID CREDENTIALS");',
'  }finally{',
...
'    renderPager();',
'  }catch(e){',
'    if(e.status===401){screen("s-login");toast("Session expired.");triggerAlarm("⚠ SESSION EXPIRED");return;}',
...
'    closeModal();',
'    await Promise.all([loadStats(),loadUsers(),loadEvents()]);',
'  }catch(e){toast("Action failed: "+e.message);triggerAlarm("⚠ ACTION FAILED");}',
'}',
...
app.get('/', (req, res) => {
  ...
  res.sendFile(INDEX_FILE, (err) => {
    if (err) {
      res.status(500).type('html').send('<pre>XYVEN WORLD — index.html not found next to server.js.</pre>');
    }
  });
});
...
app.post('/api/phone/request-otp', otpLimiter, (req, res, next) => {
  try {
    ...
    if (user.consent !== 'granted') {
      return res.status(403).json({ error: 'consent_required', message: 'Phone collection requires explicit consent.' });
    }
    ...
    if (user.otpRequests.length >= OTP_MAX_REQUESTS) {
      return res.status(429).json({ error: 'otp_throttled', message: 'Too many code requests. Please wait.' });
    }
    ...
    const payload = {
      ok: true, masked: maskPhone(country, digits),
      expiresInSeconds: Math.floor(OTP_TTL_MS / 1000),
      demo: !smsConfigured
    };
    if (!smsConfigured) {
      payload.devCode = code;
      payload.notice = 'DEMO MODE — no SMS provider configured.';
    }
    res.json(payload);
  } catch (err) { next(err); }
});
...
app.post('/api/phone/verify-otp', otpLimiter, (req, res, next) => {
  try {
    const user = ensureUserSession(req, res);
    if (!user.otp) return res.status(400).json({ error: 'no_otp', message: 'No active verification code.' });
    if (user.otp.expires < Date.now()) { user.otp = null; saveDB(); return res.status(400).json({ error: 'otp_expired', message: 'Code expired.' }); }
    if (user.otp.attempts >= OTP_MAX_ATTEMPTS) { user.otp = null; saveDB(); return res.status(429).json({ error: 'otp_locked', message: 'Too many attempts.' }); }
    const code = cleanDigits(req.body && req.body.code, 6);
    if (code.length !== 6) return res.status(400).json({ error: 'invalid_code', message: 'Enter the complete 6-digit code.' });
...
const ALLOWED_DEVICE_FIELDS = ['deviceType', 'browser', 'browserVersion', 'os', 'screenResolution', 'viewport', 'language', 'timezone', 'online'];
...
app.post('/api/device', (req, res, next) => {
  try {
    ...
    ALLOWED_DEVICE_FIELDS.forEach((key) => {
      if (body[key] !== undefined && body[key] !== null) profile[key] = cleanString(body[key], 120);
    });
    user.device = profile;
    user.lastActive = new Date().toISOString();
...
app.post('/api/admin/login', loginLimiter, (req, res, next) => {
  try {
    ...
    if (!username || !password) return res.status(400).json({ error: 'invalid_input', message: 'Username and password are required.' });
    if (password.length > 200) return res.status(400).json({ error: 'invalid_input', message: 'Invalid credentials.' });
    ...
    saveDB();
    setCookie(res, COOKIE_ADMIN, signValue(token), { maxAge: ADMIN_TTL_MS, httpOnly: true, sameSite: 'Strict' });
...
app.get('/api/admin/me', (req, res) => {
  ...
  res.json({ authenticated: true, username: admin.username, sessionId: admin.actor || 'admin', expires: admin.expires });
});

app.post('/api/admin/logout', (req, res) => {
  const cookies = parseCookies(req);
  const token = unsignValue(cookies[COOKIE_ADMIN]);
  if (token && db.adminSessions[token]) { delete db.adminSessions[token]; saveDB(); recordEvent('ADMIN LOGOUT', 'warn', null, req); }
...
app.get('/api/admin/users', requireAdmin, (req, res, next) => {
  try {
    ...
    if (search) {
      list = list.filter((u) => {
        const haystack = [
          u.id, u.sessionId, u.phone ? u.phone.country + u.phone.number : '', u.consent,
          u.device ? u.device.deviceType : '', u.device ? u.device.browser : '',
          u.device ? u.device.browserVersion : '', u.device ? u.device.os : '',
          u.device ? u.device.timezone : '', u.device ? u.device.language : ''
        ].join(' ').toLowerCase();
...
app.get('/api/admin/events', requireAdmin, (req, res, next) => {
  try {
    ...
    const events = db.events.slice(-limit).reverse().map((e) => ({
      id: e.id, label: e.label, kind: e.kind, at: e.at, userId: e.userId
    }));
    res.setHeader('Cache-Control', 'no-store');
...
/* ---------- 404 + ERRORS ---------- */
app.use((req, res, next) => {
  if (req.path.startsWith('/api/')) return res.status(404).json({ error: 'not_found', message: 'Endpoint not found.' });
  next();
});

app.get(/^\/(?!api\/|admin).*/, (req, res) => {
  res.setHeader('Cache-Control', 'no-cache');
  res.sendFile(INDEX_FILE, (err) => { if (err) res.status(404).type('text').send('Not found'); });
});

app.use((err, req, res, next) => {
  if (res.headersSent) return next(err);
  if (err && err.type === 'entity.parse.failed') return res.status(400).json({ error: 'invalid_json', message: 'Malformed request body.' });
  if (err && err.type === 'entity.too.large') return res.status(413).json({ error: 'payload_too_large', message: 'Request body is too large.' });
  console.error('[XYVEN] Unhandled error:', err && err.message ? err.message : err);
  if (req.path && req.path.startsWith('/api/')) return res.status(500).json({ error: 'server_error', message: 'An internal error occurred.' });
...
function shutdown(signal) {
  console.log('\n[XYVEN] ' + signal + ' received. Persisting datastore…');
  saveDBNow();
  server.close(() => { console.log('[XYVEN] Shutdown complete.'); process.exit(0); });
  setTimeout(() => process.exit(0), 5000).unref();
}

process.on('SIGTERM', () => shutdown('SIGTERM'));
process.on('SIGINT', () => shutdown('SIGINT'));
process.on('unhandledRejection', (reason) => { console.error('[XYVEN] Unhandled rejection:', reason); });
process.on('uncaughtException', (err) => { console.error('[XYVEN] Uncaught exception:', err); saveDBNow(); });
